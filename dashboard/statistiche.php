                                    ...
                                    <div id="tableOrari" style="overflow-x:auto;">
                                        ...
                                        ?>
                                            ...
                                            while (strtotime($date1) <= strtotime($lastDay)) {
                                                ...
                                                $risultato = $risultato . ", sec_to_time(sum((case when DATE_FORMAT(ora, '%d/%m/%Y') = '" . $date3 . "' then timestampdiff(second, ora, stopx) else 0 END)) - ifnull(0, (
                                                    SELECT timestampdiff(second, inizio, fine) as Minuti FROM stlgroup.Ferieepermessi where idoperatore = principale.idoperatore and DATE_FORMAT(inizio, '%d/%m/%Y') = '" . $date3 . "' and accettato = 1 and scalcolo = 1))) as '" . $date2 . "'";
                                                $date1 = date("d-m-Y", strtotime("+1 day", strtotime($date1))); //Adds 1 day onto current date
                                            }
                                            // $risultato = str_replace('-', '/', $risultato);
                                            $query = "WITH principale as (Select id, idoperatore, ora, Stopx from (SELECT ts.id, ts.idoperatore,
                                            (case when ts.abbuono = 1 then concat(DATE_FORMAT(ts.ora,'%Y-%m-%d') ,' ', DATE_FORMAT(ts.orarioutente, '%T')) else ts.ora end) as ora,
                                            (Select (case when ta.abbuono = 1 then concat(DATE_FORMAT(ta.ora,'%Y-%m-%d') ,' ', DATE_FORMAT(ta.orarioutente, '%T')) else ta.ora end) 
                                            from stlgroup.Oraribadge ta
                                            inner join stlgroup.Orariutenti usa on ta.idoperatore = usa.idoperatore and ta.idgiorno = usa.idgiorno
                                            where ta.id > ts.id and ta.idoperatore = ts.idoperatore and tipo = 'stop' limit 1
                                            ) as Stopx FROM stlgroup.Oraribadge ts inner join stlgroup.Orariutenti us on ts.idoperatore = us.idoperatore and ts.idgiorno = us.idgiorno where tipo = 'start')
                                            as gg WHERE ora < stopx group by id, idoperatore, ora, Stopx)
                                            select idoperatore as Id, ut.nome as Nome, ut.cognome as Cognome, se.citta as Sede" . $risultato . "
                                            from principale LEFT JOIN stlgroup.Utenti ut ON idoperatore = ut.id LEFT JOIN stlgroup.Sedi se ON ut.idsede = se.id 
                                            where stopx is not null group by idoperatore, ut.nome, ut.cognome, se.citta;";
                                            // echo $query;
